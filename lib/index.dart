// Export pages
export '/for_delete/choose_category/choose_category_widget.dart'
    show ChooseCategoryWidget;
export '/news_detail_video/news_detail_video_widget.dart'
    show NewsDetailVideoWidget;
export '/news_detail_image/news_detail_image_widget.dart'
    show NewsDetailImageWidget;
export '/for_delete/radio_page/radio_page_widget.dart' show RadioPageWidget;
export '/for_delete/career/career_widget.dart' show CareerWidget;
export '/for_delete/career_detail/career_detail_widget.dart'
    show CareerDetailWidget;
export '/profile_pages/profile/profile_widget.dart' show ProfileWidget;
export '/for_delete/menu/menu_widget.dart' show MenuWidget;
export '/for_delete/radio/radio_widget.dart' show RadioWidget;
export '/menu_pages/news_list/news_list_widget.dart' show NewsListWidget;
export '/for_delete/video_list/video_list_widget.dart' show VideoListWidget;
export '/profile_pages/admin/admin/admin_widget.dart' show AdminWidget;
export '/profile_pages/admin/imagenews/imagenews_widget.dart'
    show ImagenewsWidget;
export '/profile_pages/admin/video_news/video_news_widget.dart'
    show VideoNewsWidget;
export '/for_delete/news_list_favorite_o_l_d/news_list_favorite_o_l_d_widget.dart'
    show NewsListFavoriteOLDWidget;
export '/for_delete/video_list_favorite/video_list_favorite_widget.dart'
    show VideoListFavoriteWidget;
export '/profile_pages/admin/news_list_admin/news_list_admin_widget.dart'
    show NewsListAdminWidget;
export '/profile_pages/admin/job_admin/job_admin_widget.dart'
    show JobAdminWidget;
export '/profile_pages/admin/police/police_widget.dart' show PoliceWidget;
export '/log_in_pages/login/login_widget.dart' show LoginWidget;
export '/log_in_pages/registration/registration_widget.dart'
    show RegistrationWidget;
export '/log_in_pages/welcome_slider/welcome_slider_widget.dart'
    show WelcomeSliderWidget;
export '/menu_pages/search_page/search_page_widget.dart' show SearchPageWidget;
export '/profile_pages/main/main_widget.dart' show MainWidget;
export '/menu_pages/news_list_favorite/news_list_favorite_widget.dart'
    show NewsListFavoriteWidget;
export '/menu_pages/news_video_list_new/news_video_list_new_widget.dart'
    show NewsVideoListNewWidget;
export '/menu_pages/reset_password/reset_password_widget.dart'
    show ResetPasswordWidget;
export '/menu_pages/notification/notification_widget.dart'
    show NotificationWidget;
export '/profile_pages/admin/comments_list/comments_list_widget.dart'
    show CommentsListWidget;
export '/profile_pages/admin/comments_detail/comments_detail_widget.dart'
    show CommentsDetailWidget;
export '/menu_pages/choosed_cat/choosed_cat_widget.dart' show ChoosedCatWidget;
export '/profile_pages/new_profile_settings/new_profile_settings_widget.dart'
    show NewProfileSettingsWidget;
